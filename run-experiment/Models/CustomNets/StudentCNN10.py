...
class StudentDense256:
    ...
    def buildAndCompile(self):
        ...
        self.student.add(Conv2D(32, kernel_size=(3, 3),
                                activation='relu',
                                input_shape=self.input_shape))
        self.student.add(Conv2D(64, (3, 3), activation='relu'))
        self.student.add(MaxPooling2D(pool_size=(2, 2)))
        #self.student.add(Dropout(0.25))  # For reguralization
        self.student.add(Flatten())
        self.student.add(Dense(64, activation='relu'))
        #self.student.add(Dropout(0.5))  # For reguralization
        self.student.add(Dense(self.nb_classes))
        self.student.add(Activation('softmax'))  # Note that we add a normal softmax layer to begin with

        self.student.compile(loss='categorical_crossentropy',
