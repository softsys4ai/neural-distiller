...
def run():
    ...
    log_dir = os.path.join(log_dir, f"ESKD_baseline_noise_and_adv_evaluation_{cfg.dataset}_{cfg.student_model_size}_{now_datetime}")
    os.mkdir(log_dir)
    ...
    for name in STUDENT_MODEL_NAMES:
        size, interval, test_acc, train_acc = re.findall(rf"model_(\d+)_(\d+)\|\d+_(\d+.\d+)_(\d+.\d+).", name)[0]
    ...
    for i in range(len(cfg.SIGMA_VALS4)):
        ...
        for j in range(len(X_test)):
            X_test_gauss_noised[j] = X_test[j] + np.random.normal(cfg.MEAN4, cfg.SIGMA_VALS4[i], (X_test[j].shape[0],
                                                                                                X_test[j].shape[1],
                                                                                                X_test[j].shape[2]))
        for j in range(len(X_train)):
            X_train_gauss_noised[j] = X_train[j] + np.random.normal(cfg.MEAN4, cfg.SIGMA_VALS4[i], (X_train[j].shape[0],
                                                                                                  X_train[j].shape[1],
                                                                                                  X_train[j].shape[2]))
        ...
        for i in range(max(len(cfg.EPS_VALS4), len(cfg.SIGMA_VALS4))):
            if cfg.USE_ADV_ATTACK:
                if i < len(cfg.EPS_VALS4):
                    ...
                    student_art_model = KerasClassifier(model=curr_student_model, clip_values=(dataset_min, dataset_max),
                                                        use_logits=False)
