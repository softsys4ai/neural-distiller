                        ...
                        # if no previously trained model, train the network
                            ...
                            if teacher_train_logits is None or teacher_test_logits is None:
                                ...
                                model.compile(optimizer=optimizer,
                                              ...
                                              metrics=['accuracy'])
                                # evaluate network
                                train_score, val_score = helper_util.calculate_unweighted_score(logger, model, X_train,
                                                                                               Y_train,
                                                                                               X_test, Y_test)
                                # save evaluation
                                ...
                                if len(order) != 1:
                                    logger.info("creating student training data...")
                                    teacher_train_logits, teacher_test_logits = teacher_utils.createStudentTrainingData(model, temp, X_train,
                                                                                                     Y_train, X_test,
                                                                                                     Y_test)
