...
RESULTS_FILE = os.path.join(cfg.processed_csv_path, "logit_analysis.csv")
EXP = "ESKD_teacher"
CMAP = "gnuplot2"
...
if INCLUDE_ENTROPY:
    ...
    min_soft_diffs = np.zeros(len(epochs))
    mean_soft_diffs_train = np.zeros(len(epochs))
    mean_soft_diffs_test = np.zeros(len(epochs))
    for i in range(len(epochs)):
        temp_df = df[df.interval == epochs[int(i)]]
        mean_soft_diffs_train[i] = temp_df['avg_train_entropy'].mean()

    # plot min, mean, max of soft differences
    plt.figure(0)
    # plt.plot(epochs, mean_soft_diffs_train, label="Entropy Value", color="k")
    entropy_plot = sns.lineplot(x="teacher_interval", y="avg_train_entropy", legend='brief', label="train_entropy", data=df)
    entropy_plot = sns.lineplot(x="teacher_interval", y="avg_test_entropy", legend='brief', label="test_entropy", data=df)
    # plt.hlines(0., min_epoch, max_epoch, colors="r", linestyles="dashed", label="Baseline Max test AR Accuracy")
    # plt.hlines(0., min_epoch, max_epoch, colors='k', linestyles="dashed", label="Baseline Mean test AR Accuracy")
    plt.xlabel("Epoch Interval")
    epochs = np.arange(0, max_epoch+epoch_interval-1e-2, epoch_interval)
