...
def plot_epoch_against_vacc(min_epoch=0, max_logit_epochs=200, max_epoch=200, epoch_interval=10):
    ...
    plt.savefig(os.path.join(cfg.figures_path, "max_mean_min.png"))
    plt.show()
    #
...
def plot_epoch_against_temp_against_vacc(min_epoch=0, max_logit_epochs=100, max_epoch=200, epoch_interval=10,
    ...
    ax.set_xticks([i+0.5 for i in np.arange(0, 20+1, 1)])
    ax.set_yticks([i+0.5 for i in np.arange(0, 10+1, 1)])
    ...
    plt.savefig(os.path.join(cfg.figures_path, plotname))
