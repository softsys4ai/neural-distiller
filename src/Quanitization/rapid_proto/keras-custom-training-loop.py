...
# np.random.seed(0)
# tf.set_random_seed(np.random.seed(0))
...
grads = optimizer.get_gradients(params=model.trainable_weights, loss=loss)
updates_op = optimizer.get_updates(
    params=model_q.trainable_weights,
    loss=loss)


trainable_weights_from_model = model.trainable_weights
...
for epoch in range(epochs):
    ...
    for idx in pbar:
        ...
        trainable_weights_from_model = model.trainable_weights
        for trainable_weight in trainable_weights_from_model:
            # weight = trainable_weight.get_weights
